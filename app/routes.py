...
def stats():
    ...
    stat2 = []
    #stat2 list that contains usernames and scores
    stat2.append(stat_user)
    stat2.append(stat_score)
    ...
    scores_list = scores_list_str.split(",")
    ...
    users_list = [each.username for each in (User.query.with_entities(User.username))]
    for u in User.query.with_entities(User.difficulty_dict):
        ...
        if type(dic2) == str:
            ...
            stat3.append(dic_diff)
    
    size_dic = len(stat3)
    
    return render_template("HTML/statistics.html", title = "Statistics", stat2 = stat2, length = length, i = i, scores_list = scores_list, size = size, total_score = total_score, stat3 = stat3, size_dic = size_dic, users_list = users_list)
